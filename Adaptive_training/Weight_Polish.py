...
data = np.reshape(data[0:4500,:,:],[-1,1024])
...
data_label = np.eye(num_class)[np.squeeze(data_label[0:4500]).reshape(-1)].astype(np.float32)
...
weight_fc1_bin = np.load('./output/model_bin_fc1_222_6628%.npy')
weight_fc2 = np.load('./output/model_fc2_222_6628%.npy')
...
#  #temp = np.reshape(np.reshape(weight_fc1_bin[:,i],[32,32]).T,[1024])
#  if (all_sum[i] > 50):
#    temp[(1024-(abs(int(all_sum[i]/10)))):]=-1
#    temp[0:(abs(int(all_sum[i]/10)))]=-1
#  if (all_sum[i] < -50):
#    temp[(1024-(abs(int(all_sum[i]/7)))):]=1
#    temp[0:(abs(int(all_sum[i]/7)))]=1
#  weight_fc1_bin[:,i]=temp
#  #weight_fc1_bin[:,i]=np.reshape(np.reshape(temp,[32,32]).T,[1024])
