...
data = np.reshape(data[0:27000,:,:],[-1,1024])
...
data_label = np.eye(num_class)[np.squeeze(data_label[0:27000,:]).reshape(-1)].astype(np.float32)
...
data_test = np.reshape(data_test[0:4500,:,:],[-1,1024])
...
data_test_label = np.eye(num_class)[np.squeeze(data_test_label[0:4500]).reshape(-1)].astype(np.float32)
...
with tf.device('/gpu:0'):
    ...
    sig_coef = tf.compat.v1.placeholder("float")

    bin_W_fc1 = tf.sigmoid(W_fc1 * sig_coef) * 2.0 - 1.0
    # bin_W_fc1 = W_fc1
    # bin_W_fc1 = binarize(W_fc1)
    h_fc1 = tf.matmul(x, bin_W_fc1)
    h_fc1 = tf.clip_by_value(h_fc1,-256,256)
    bin_h_fc1 = tf.sigmoid(h_fc1 * sig_coef) * 2.0 - 1.0
    # bin_h_fc1 = tf.sign(h_fc1)
    # bin_h_fc1 = binarize(h_fc1)
    bin_h_fc1 = bin_h_fc1[:,0:10]
    y = tf.nn.softmax(bin_h_fc1)

    bin_W_fc1_test = binarize(W_fc1)
    h_fc1_test = tf.matmul(x, bin_W_fc1_test)
    h_fc1_test = tf.clip_by_value(h_fc1_test,-256,256)
    bin_h_fc1_test = binarize(h_fc1_test)
    bin_h_fc1_test = bin_h_fc1_test[:,0:10]
    y_test = tf.nn.softmax(bin_h_fc1_test)
    ...
    correct_prediction_test = tf.equal(tf.argmax(y_test,1), tf.argmax(y_,1))
    ...
    while(max_abs_sum>10):
        ...
        if Train==True:
            for i in range(10000):
                ...
                train_step.run(feed_dict={x: train_img, y_: train_label, sig_coef: coef_tmp})

                if i%200 == 0:
                    train_accuracy = accuracy.eval(feed_dict={
                        x: data[:,:1024], y_: data[:,1024:], sig_coef: coef_tmp})
                    ...
                    if test_accuracy > max_test_acc:
                        ...
                        weight=sess.run(bin_W_fc1_test)
                        all_sum = np.zeros(16)
                        for k in range(16):
                            all_sum[k] = np.sum(weight[:,k])
                            # print (all_sum[i])
                        max_abs_sum = np.max(abs(all_sum))
                        if max_test_acc > 0.45:
                            np.save('./output/model_bin_fc1_'+str(int(max_test_acc*10000))+'%'+'_'+str(int(max_abs_sum))+'.npy',weight)
        ...
        acc = accuracy.eval(feed_dict={x: data[:,:1024], y_: data[:,1024:], sig_coef: coef_tmp})
        print ("train accuracy %g"%(acc))

        acc_test = accuracy_test.eval(feed_dict={x: data_test[:,:1024], y_: data_test[:,1024:]})
